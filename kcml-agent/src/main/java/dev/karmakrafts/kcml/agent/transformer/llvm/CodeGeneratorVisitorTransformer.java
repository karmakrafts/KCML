...
public final class CodeGeneratorVisitorTransformer extends AbstractClassTransformer {
    ...
    protected boolean shouldTransform(final String className) {
        return className.equals(Konan.CODE_GENERATOR_VISITOR.getInternalName());
    }
    ...
    private boolean transformEvaluateFunctionCall(final MethodNode methodNode) {
        ...
        return ASMUtils.stream(methodNode.instructions)
            ...
            .findFirst()
            .map(needle -> {
                logger.info("Transforming %s%s (onEvaluateFunctionCall)", methodNode.name, methodNode.desc);
                ...
                builder.invokestatic(KCML.LLVM_HOOKS, false, "onEvaluateFunctionCall", CInterop.C_POINTER, Types.OBJECT, Common.IR_CALL, Types.LIST);
                builder.dup();
                builder.ifnull("kcml_fallthrough");
                builder.areturn();
                builder.label("kcml_fallthrough");
                builder.pop();
                methodNode.instructions.insert(needle, builder.build());
                return true;
            })
            .orElse(false);
        // @formatter:on
